...
class CedElkjopnordicFeed extends ObjectModel
{
    public static $definition = array(
        'table' => 'cedelkjopnordic_products_feed',
        'primary' => 'id',
        'multilang' => false,
        'fields' => array(
            ...
            'transform_lines_in_succes' => array('type' => self::TYPE_STRING, 'db_type' => 'text'),
            'transform_lines_read' => array('type' => self::TYPE_STRING, 'db_type' => 'text'),
            'import_status' => array('type' => self::TYPE_STRING, 'db_type' => 'text'),
    ...
    public $transform_lines_in_succes;
     ...
     * @return array
     */
    public function deleteProductFeed($id)
    {
        try {
            $db = Db::getInstance();
            $sql = "SELECT * FROM " . _DB_PREFIX_ . "cedelkjopnordic_products_feed WHERE import_id='" .(int)$id . "'";
            $res = $db->executeS($sql);
            if ($res) {
                ...
                $query = "DELETE FROM " . _DB_PREFIX_ . "cedelkjopnordic_products_feed WHERE import_id='" . $id . "'";
    ...
    public function updateFeed($id)
    {
        try {
            $db = Db::getInstance();
            $sql = "SELECT feed_file from " . _DB_PREFIX_ . "cedelkjopnordic_products_feed WHERE import_id='" . (int)$id . "'";
            $res = $db->executeS($sql);
            $feed_file = '';
            if (isset($res[0]['feed_file'])) {
                $feed_file = $res[0]['feed_file'];
            ...
            $result = $elkjopnordicProduct->syncFeed($id, 'products', $feed_file);
            return $result;
        } catch (\Exception $e) {
            return array('success' => false, 'message' => $e->getMessage());
        }
